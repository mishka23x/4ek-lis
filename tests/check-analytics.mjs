...
const compat = read('google-apps-script/Compat.gs');
...
new vm.Script(compat, { filename: 'Compat.gs' });
...
assert(!apps.includes('Authorization:'), 'modern Supabase secret must not be sent as bearer authorization');
assert(compat.includes('function URL(value)'), 'Apps Script URL compatibility shim missing');
...
assert(sql.includes('submission_id uuid primary key'), 'submission idempotency primary key missing');
